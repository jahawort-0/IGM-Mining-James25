...
if (ischar(prior_ind))              %%Error here, need prior_ind from prior catalog or filter flags
...
%     % in read_spec_DESI.m
    ...
    if SNR<SNR_threshhold
        fprintf('skipping quasar %i/%i (z_QSO = %0.4f)  because of low S/N...\n', ...
                              all_quasar_ind, num_quasars, z_qso)
        continue
    end
    ...
    fprintf('\n');
    fprintf('prior absorber probabilities')
    for i = 1:max_civ
    ...
    % Chi^2 calc, assess quality of continuum fit
    if (ChiSquare == 1)
        chisquare_DESI_CIV
    end
...
if saving==1
    variables_to_save = {'training_set_name', ...
        ...
        'map_z_c4L1', 'map_N_c4L1', 'map_sigma_c4L1','all_B','fit_chi2'};
