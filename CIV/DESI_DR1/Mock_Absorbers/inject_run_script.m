...
if cataloging == 1
    inject_build_catalog_DESI_dr1_C4
end
...
if preloading == 1
    preload_qsos_DESI_dr1_C4
end
load(sprintf('%s/preloaded_qsos_%s', processed_directory(releaseTest), testing_set_name));
    
% load preprocessed mock absorbers
fprintf('Preloading QSOs ...\n')
if preloading == 1
    inject_preload_qsos_DESI_dr1_C4
end
load(sprintf('%s/preloaded_qsos_%s', processed_directory(releaseTest), testing_set_name));
...
if processing==1
    ...
    %parpool('local', cores);
    inject_process_qsos_DESI_dr1_C4
