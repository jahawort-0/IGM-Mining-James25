...
z_PM_test = all_z_civ_C13(test_ind,:);
j0=0;
...
for quasar_ind = 1:numel(all_wavelengths)
    ...
    this_isAbs_15       =     all_isAbs{quasar_ind,1};
    this_isAbs_20       =     all_isAbs{quasar_ind,2};
    this_isAbs_30       =     all_isAbs{quasar_ind,3};
    this_isAbs_40       =     all_isAbs{quasar_ind,4};
    this_isAbs_50       =     all_isAbs{quasar_ind,5};
    ...
    masked_abs_pixels(quasar_ind,4) = nnz(this_isAbs_40 & masked_regions)/nnz(this_isAbs_40);
    masked_abs_pixels(quasar_ind,5) = nnz(this_isAbs_50 & masked_regions)/nnz(this_isAbs_50);
...
for num_c4=1:max_civ
        ...
        if(num_c4>1)
            ...
            % end
            if (p_c4(this_quasar_ind, num_c4-1)>0.75) %%<- 0.75
                ind_not_remove = ind_not_remove  & ...
                    (abs(this_z_1548 - map_z_c4L2(this_quasar_ind, num_c4-1))>kms_to_z(dv_mask)*(1+map_z_c4L2(this_quasar_ind, num_c4-1))) & ...
                    (abs(this_z_1550 - map_z_c4L2(this_quasar_ind, num_c4-1))>kms_to_z(dv_mask)*(1+map_z_c4L2(this_quasar_ind, num_c4-1)));
            end
       ...
       % compute sample probabilities and log likelihood of singlet L1 model in
        ...
        [B,chi,absorber_gate_mask] = absorber_gate(this_flux,this_wavelengths,this_mu,c4_muL2);
        all_B(this_quasar_ind,num_c4) = B;
        %B_gate = (B <= 0.85);
        ...
        if ((plotting == 1) && (num_c4 == 1))
            ...
            ttl = sprintf('ID:%s, zQSO:%.2f\n P(CIV)=%.2f, z_{CIV}=%.3f, P(S)=%.2f, REW=%.3f, SN=%.2f, sigma=%.3f, N=%.2f,B=%.3f ,Chi^2=%.3f', ...
                ...
                ,all_B(this_quasar_ind,num_c4),fit_chi2(this_quasar_ind))
            ...
            pltQSO(this_flux, this_wavelengths, this_mu, c4_muL2, c4_muL1, this_noise_variance, ttl, fid, flagged_pix,ind_not_remove,...
                normalization_min_lambda, normalization_max_lambda,z_qso,absorber_gate_mask)
